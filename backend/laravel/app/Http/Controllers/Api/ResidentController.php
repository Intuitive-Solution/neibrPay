...
class ResidentController extends Controller
{
    ...
    public function sendActivation(Request $request, string $id): JsonResponse
    {
        ...
        if (!$resident->is_active || $resident->deleted_at) {
            return response()->json([
                'message' => 'Cannot send activation email to inactive or deleted residents'
            ], 400);
        }
        ...
        if (!$n8nWebhookUrl) {
            Log::warning('N8N_WEBHOOK_URL not configured. Activation email webhook not sent.');
            return response()->json([
                'message' => 'Email service not configured. Activation email not sent.',
            ], 500);
        }
        ...
        try {
            ...
            $signupLink = rtrim($appUrl, '/') . '/signup?' . $signupParams;
            ...
            Log::info('Sending activation email webhook request', [
                'url' => $n8nWebhookUrl,
            ...
            try {
                ...
                if ($response->successful()) {
                    Log::info('Activation email webhook sent successfully', [
                    ...
                    if ($response->status() === 404) {
                        ...
                        Log::warning('Activation email webhook returned non-success status', [
                            'status' => $response->status(),
                ...
                Log::error('Failed to send activation email webhook: ' . $e->getMessage(), [
                    'resident_id' => $resident->id,
                    'resident_email' => $resident->email,
                    'error' => $e->getMessage(),
                ]);
            ...
            Log::error('Error in sendActivation: ' . $e->getMessage(), [
                'resident_id' => $id,
                'error' => $e->getMessage(),
            ]);
