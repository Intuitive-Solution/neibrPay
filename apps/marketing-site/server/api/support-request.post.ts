...
const reasonLabels: Record<string, string> = {
  ...
  'account-setup': 'Account Setup/Onboarding',
  'invoice-payment': 'Invoice/Payment Issues',
...
export default defineEventHandler(async event => {
  ...
  const { name, email, communityName, reason, callbackNumber } = body;

  if (!name || !email || !communityName || !reason || !callbackNumber) {
    throw createError({
      statusCode: 400,
  ...
  const safeCallbackNumber = escapeHtml(callbackNumber);
  const safeReasonLabel = escapeHtml(reasonLabel);

  // Email content
  const mailOptions = {
    from: `"NeibrPay Support" <${config.smtpUser}>`,
    to: config.supportEmail,
    replyTo: email,
    subject: `Support Request: ${reasonLabel} - ${communityName}`,
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          <div class="container">
            <div class="header">
              ...
              <p style="margin: 10px 0 0 0; opacity: 0.9;">A callback has been requested</p>
            </div>
            <div class="content">
              <div class="field">
                <div class="field-label">Reason for Contact</div>
                <div class="field-value">${safeReasonLabel}</div>
              </div>
              ...
              <div class="field">
                ...
                <div class="field-value">
                  <a href="tel:${safeCallbackNumber}">${safeCallbackNumber}</a>
                </div>
              </div>
              ...
              <div class="footer">
                ...
                <p>This is an automated notification from the NeibrPay support form.</p>
              </div>
            </div>
          </div>
        </body>
    ...
    `,
    text: `
New Support Request - Callback Requested

Reason for Contact: ${reasonLabel}
Name: ${name}
Email: ${email}
...
This is an automated notification from the NeibrPay support form.
    `.trim(),
  };
  ...
  try {
    ...
    return {
      success: true,
      message:
        'Your support request has been received. We will call you back soon.',
    };
  } catch (error: any) {
