...
const handleSubmit = async () => {
  if (!props.payment || !action.value) return;
  ...
  try {
    ...
    if (action.value === 'approve') {
      payment = await approvePaymentMutation.mutateAsync({
      ...
      payment = await rejectPaymentMutation.mutateAsync({
        paymentId: props.payment.id,
        data: {
    ...
    if (error.response?.data?.errors) {
      ...
      errors.value.general =
        error.message || 'Failed to process payment review';
    }
  } finally {
    isSubmitting.value = false;
  }
